...
class REST
{
    ...
    {
        $guep = RiskManager::where('name', 'GUEP')->first();
        if (is_null($guep->sessions->last()) || now() > $guep->sessions->last()->expire_at) {
            try {
                $formData = [
                    'usuario'        => $this->user,
                    'senha'          => $this->password,
                ...
                $newSession = $guep->sessions()->updateOrCreate([
